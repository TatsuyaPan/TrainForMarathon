...
export function serializeWorkout(
  workout: Workout,
  options?: { version?: boolean; targetMode?: "zone" | "pace"; paces?: TrainingPaces | null },
): string;
```

`parseWorkoutDsl` 优先使用文本中的显式版本，否则使用 `options.defaultVersion`，再否则使用 `CURRENT_WORKOUT_DSL_VERSION`。平台也可以绕过统一入口，直接调用具体版本解析器。

**版本号是规范的一部分，不随平台功能升级。** 只有冻结规范本身需要扩展语法时才新增版本目录，
并且必须先取得规范维护者的确认；平台侧的新功能（例如下面的配速写法）只允许在既有版本语法之内实现。
语法事实来源是冻结规范 `spec/dsl/v1/workout-dsl-v1.md`，本文件只保留设计取舍。
...
同一份课表可以导出两种写法，两种都在 v1 语法之内：

```text
MS:6x(8min@T+90s@jog)                    # 档位写法（默认，规范往返口径）
MS:6x(8min@P3:45-4:00/km+90s@jog)       # 配速写法（按当前能力换算）
```

- 档位写法是事实来源：`@T` 表示「乳酸阈强度」，与谁在跑无关；
- 配速写法是**派生投影**：用当前能力把 `@T` 换算成 `@P3:45-4:00/km`，让跑者知道这次该跑多快；
- 投影是**单向**的：档位 → 配速可以换算，配速 → 档位不行。`@P4:45-5:00/km` 本身就是「明确配速」，
  反推会凭空发明训练意图，因此只有含档位的课表才提供配速写法，明确配速在任何写法下都保持原样；
- 缺少能力、ST 步骤或换算结果越界（不在 90–900 秒/公里、快端不小于慢端）时该步骤保持档位写法，不伪造配速；
- 配速写法**不是规范往返口径**：它导入回来是明确配速，不再是原来的档位。需要长期交换时用档位写法；
- 实现落在 `packages/core/src/dsl/pace-projection.ts`（纯函数投影）+ `serializeWorkout(workout, { targetMode: "pace", paces })`，
  **不新增、不修改任何冻结版本**。

界面上的写法开关见交互设计文档 §19.4。
...
课表展示处提供两个同源的开关：**展示口径**（怎么读课表）与 **DSL 写法**（怎么导出文本）。
写法开关产出强度版或配速版（§6.7），与展示口径一一对应；界面开关、默认值与验收标准见交互设计文档 §19。
...
- 内置课程全部通过解析、校验和往返测试；
- 复制课程生成独立深拷贝。
- 档位写法导出后结构不变，且版本行仍是 `WORKOUT/1`；
- 配速写法只替换档位目标：明确配速、心率与 RPE 步骤保持原样（含嵌套循环与 ST）；
- 缺少能力或换算越界时配速写法退回档位写法；
- 投影是纯函数：不修改入参，返回独立对象；
- 明确配速不会被反推成档位（`workoutHasExplicitPace` / `workoutHasIntensity`）。
...
10. 导入配速版 DSL，在课程库里切到强度版再切回配速版，两种写法重新导入后结构一致。
...
- 课程结构能够无损导出和重新解析；
- 强度版与配速版是同一份课表的两种写法：导入配速版等价于导入强度版，导出配速版按当前能力重算；
