...
**版本：** 1
**状态：** 冻结（本文件不再修改；语义变更进入 v2）
**冻结日期：** 2026-09-10
**适用入口：** `parseWorkoutDslV1` / `serializeWorkoutV1` / `CURRENT_WORKOUT_DSL_VERSION`

本文档是版本 1 的完整、自包含规范。任何平台的 v1 解析器、序列化器、编辑器与展示实现都以本文为准。
...
- 坡跑用 `inclinePercent` 表达，不是独立强度。
- `H` 不是 v1 的强度记号；训练思想正文中的 `H` 记法（与 `I` 同一强度，基于时间控制）自 v2 起支持。
- 每个跑步步骤只有一个主目标；`rpe` 字段在已存在主目标时只作为辅助体感提示。
- v1 不允许同一个跑步步骤同时设置配速主目标和心率主目标。
...
本文件发布后即为版本 1 的冻结文本。任何语义变更（新增步骤类型、修改关键字、放宽或收紧约束、改变规范化结果）都必须：

1. 在 `spec/dsl/v2/` 建立新规范；
2. 在 core 中新增 `dsl/v2.ts` 并注册到版本表；
3. 保持 v1 解析器与序列化器不变，除非是事实性笔误修正。
